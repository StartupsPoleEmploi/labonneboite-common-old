...
from labonneboite_common import csv
...
class UserFavoriteOffice(CRUDMixin, Base):
    ...
    @classmethod
    def add_favorite(cls, user: User, office: Office, rome_code: Optional[str] = None):
        """
    ...
    def user_favs_as_csv(cls, user):
        ...
        if not user.is_anonymous:
            writer.writerows(fav.as_csv_row() for fav in db_session.query(cls).filter_by(user_id=user.id))
