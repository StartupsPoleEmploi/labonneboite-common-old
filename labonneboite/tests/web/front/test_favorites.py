...
class FavoriteTest(FavoriteBaseTest):
    ...
    def test_favorites_add(self):
        ...
        url_list = self.url_for('user.favorites_list')
        url_add = self.url_for('user.favorites_add', siret=office.siret, rome_code=rome_code)
    ...
    def test_favorites_add_without_rome_code(self):
        ...
        self.assertEqual(0, UserFavoriteOffice.query.filter(UserFavoriteOffice.user_id == self.user.id).count())
        with self.test_request_context():
            self.login(self.user)
