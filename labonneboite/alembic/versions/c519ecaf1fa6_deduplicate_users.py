...
def deduplicate_users():
    ...
    from labonneboite_common.models import auth
    from labonneboite_common.models.user_favorite_offices import UserFavoriteOffice
    ...
    for user in auth.User.query.group_by('external_id').having(sa.func.count(auth.User.external_id) > 1):
        ...
        for duplicate_user in auth.User.query.filter(auth.User.external_id == user.external_id,
        ...
        print("Removing {} duplicates for user #{} ({} favorite added to original user)".format(
            len(duplicate_user_ids), user.id, favorite_count))
        # Remove duplicate social user
        db_session.query(auth.UserSocialAuth).filter(
            auth.UserSocialAuth.user_id.in_(duplicate_user_ids)).delete(synchronize_session=False)
