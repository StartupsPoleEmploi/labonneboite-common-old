...
from labonneboite_common import load_data
...
def load_csv_perf_prediction_and_effective_h(filename, delimiter=';'):
    #date_time_obj = datetime.datetime.strptime(date_time_str, '%Y-%m-%d %H:%M:%S.%f')
    for row in load_data.load_csv_file(filename, delimiter):
        perf_importer_cycle_info = PerfPredictionAndEffectiveHirings(_id=row[0],
                                                                     importer_cycle_infos_id=row[1],
                                                                     siret=row[2],
                                                                     naf=row[3],
                                                                     city_code=row[4],
...
def get_sum_predict(pdf, row, i, colonne, nom=None, is_lbb=True):
    logger.info(
        f"START - {str(datetime.now())}- is_lbb : {is_lbb} - computing sum predict by {colonne} for cycle importer {row['cycle']} and sum {i}"
    )
    if nom is not None:
        row_nom = row[nom]
    else:
    ...
    if i == 10:
        ...
        if colonne != "global":
            ...
            pdf.head_predict[row["cycle"]][row[nom]] = {"len": head_len, "sum": head_sum}
        else:
            ...
            if row["cycle"] not in pdf.head_predict_global:
                pdf.head_predict_global[row["cycle"]] = {"len": head_len, "sum": head_sum}
    ...
    if i == 100:
        rmse_list = numpy.array([
            calcul_rmse(eff, pred)
            for eff, pred in zip(numpy.array(df_predict["effective"]), numpy.array(df_predict["predict"]))
        ])
        logger.info(
...
def lancement_requete(pdf, colonne, nom=None, is_lbb=True):
    ...
    if len(df_nb_entreprise) > 0:
        for i in range(10, 110, 10):
            df_nb_entreprise[f'sum{i}'] = df_nb_entreprise.apply(
                lambda row: get_sum_predict(pdf, row, i, colonne, nom, is_lbb), axis=1)
        ...
        df_result['propRecrutNonLBX'] = numpy.array([
            get_prop_recrut_non_lbx(total, total_lbx) for total, total_lbx in zip(
                numpy.array(df_result["nbTotalHirings"]), numpy.array(df_result["nbTotalLBXHirings"]))
        ])
        return df_result
    else:
...
def fill_indicators_sheet(pdf, is_lbb):
    ...
    spreadsheet_id = settings.SPREADSHEET_IDS["perf_indicators_lbb"] if is_lbb else settings.SPREADSHEET_IDS[
        "perf_indicators_lba"]
    ...
    naf_sheet_report = GoogleSheetReport(service=service,
    ...
    departement_sheet_report = GoogleSheetReport(service=service,
                                                 spreadsheet_id=spreadsheet_id,
                                                 sheet_index=1,
                                                 start_cell=None,
    ...
    global_sheet_report = GoogleSheetReport(service=service,
                                            spreadsheet_id=spreadsheet_id,
                                            sheet_index=2,
                                            start_cell=None,
